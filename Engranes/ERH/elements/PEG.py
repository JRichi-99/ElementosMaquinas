...
class ParEngranesGeometria:
    ...
    def resumen_geometria(self, name=None):
        import os
        ...
        if name is None:
            os.makedirs(f"conjunto_{self.id}", exist_ok=True)
            filename = os.path.join(f"conjunto_{self.id}", f"geometria_{self.id}.txt")
        else: 
            os.makedirs(f"planetario_{self.id}", exist_ok=True)
            filename = os.path.join(f"planetario_{self.id}", f"geometria_{self.id}.txt")
