...
class ParEngranesEsfuerzo(ParEngranesTransmision):
    ...
    def resumen_esfuerzos(self, dec=2):
        import os
        ...
        lines.append(f"{'K_A':<6}: {fmt(self.K_A)}    {'K_M':<6}: {fmt(self.K_M)}    {'K_S':<6}: {fmt(self.K_S)}")
        lines.append(f"{'K_B':<6}: {fmt(self.K_B)}    {'K_I':<6}: {fmt(self.K_I)}    {'K_V':<6}: {fmt(self.K_V)}")
        lines.append(f"{'C_p':<6}: {fmt(self.C_p)} [√MPa]    {'C_F':<6}: {fmt(self.C_F)}    {'Q_v':<6}: {fmt(self.Q_v)}")
        lines.append(f"{'I (AGMA)':<10}: {self.I}    {'F (mm)':<10}: {fmt(self.F)}")
        # J desglosados
        lines.append(f"{'Jp_pin':<10}: {fmt(getattr(self,'Jp_pin', None))}    "
                    f"{'Jmod_pin':<10}: {fmt(getattr(self,'Jmod_pin', None))}    "
                    f"{'J_pin (ef)':<12}: {fmt(self.J_pin)}")
        lines.append(f"{'Jp_eng':<10}: {fmt(getattr(self,'Jp_eng', None))}    "
                    f"{'Jmod_eng':<10}: {fmt(getattr(self,'Jmod_eng', None))}    "
                    f"{'J_eng (ef)':<12}: {fmt(self.J_eng)}")
        lines.append("")
